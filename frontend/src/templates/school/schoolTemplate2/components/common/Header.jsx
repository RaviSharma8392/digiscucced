...
const STYLES = `
  ...
  /* ── Background Logic (Mobile vs Desktop) ── */
  .hdr-main-bg {
    /* Default for mobile: Solid cream */
    background: var(--cream);
  }

  @media (min-width: 640px) {
    .hdr-main-bg {
      /* Desktop: Flag image with a transparent cream overlay for readability */
      background:  
                  url('/india-flag-bg.jpg') center/cover no-repeat;
    }
  }
...
export default function Header({ schoolData = {}, navItems = [] }) {
  ...
  return (
    <>
      ...
      {/* Replaced inline style background with 'hdr-main-bg' class */}
      <header
        className={`hdr-root hdr-main-bg w-full sticky top-0 z-50 transition-all duration-300 ${
          scrolled ? "shadow-2xl" : "shadow-md"
        }`}>
        {/* ── Tricolor bar ── */}
        <div className="tricolor-bar flex h-1.5 w-full overflow-hidden relative z-20">
          <span
        ...
        {/* ── Main content ── */}
        <div className="hdr-content relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 py-4">
          {/* ── Identity row ── */}
          <div className="flex items-center gap-4">
            ...
            {schoolData.leftLogo && (
              <SafeImg
                ...
                alt="Left Logo"
                className="w-16 sm:w-20 md:w-54 h-auto object-contain flex-shrink-0 drop-shadow-md"
              />
            )}
                ...
                <div className="flex justify-center gap-3 mb-2">
                  {schoolData.smallLogos.map((logo, i) => (
                    <SafeImg
                      key={i}
                      src={logo}
                      className="h-7 sm:h-9 object-contain opacity-90 drop-shadow-sm"
                    />
                  ))}
                </div>
              )}
            ...
            {schoolData.rightLogo && (
              <SafeImg
                ...
                alt="School Logo"
                className="w-16 sm:w-20 md:w-24 h-auto object-contain flex-shrink-0 drop-shadow-md hidden sm:block"
              />
            )}

            {/* Mobile hamburger */}
            <button
              className="sm:hidden ml-1 p-2 rounded-md transition-colors hover:bg-white/50"
          ...
          {navItems.length > 0 && (
            <nav className="hidden sm:block mt-5">
              <div
                className="rounded-lg px-4 py-2 shadow-inner border border-white/10 backdrop-blur-sm"
                ...
                <ul className="flex justify-center flex-wrap gap-2">
                  {navItems.map((item) => (
                    <li key={item.id} className="hdr-nav-group relative">
                      <a
                        href={item.url || "#"}
                        className="hdr-nav-link relative flex items-center gap-1 px-4 py-2 rounded-md text-white/90 hover:text-white hover:bg-white/10 transition-all duration-200 cursor-pointer">
                        <div className="text-center">
                          ...
                          {item.hi && (
                            <div className="text-[0.65rem] text-white/60 leading-tight mt-0.5">
                              {item.hi}
                            </div>
                          )}
                        </div>
                        {item.children && (
                          <ChevronDownIcon className="w-4 h-4 opacity-70 mt-0.5 flex-shrink-0" />
                        )}
                      </a>
                      ...
                      {item.children && (
                        <ul
                          className="hdr-dropdown absolute left-0 top-full mt-2 w-56 rounded-lg overflow-hidden z-50"
                          style={{
                            background: "white",
                            boxShadow: "0 10px 40px rgba(26,31,94,0.15)",
                          ...
                          {item.children.map((child, i) => (
                            <li
                              ...
                              <a
                                href={child.url || "#"}
                                className="flex items-center gap-3 px-4 py-3 text-[0.9rem] transition-colors hover:bg-[#fef3e2] group"
                                style={{
                                  color: "var(--navy)",
                                  fontFamily: "'Rajdhani', sans-serif",
                                  fontWeight: 600,
          ...
          {mobileOpen && navItems.length > 0 && (
            <div
              className="hdr-mobile-nav sm:hidden mt-4 overflow-hidden rounded-lg bg-white shadow-lg"
              ...
              <ul className="divide-y divide-gray-100">
                {navItems.map((item) => (
                  <li key={item.id}>
                    <a
                      href={item.url || "#"}
                      className="flex items-center justify-between px-5 py-3.5 transition-colors hover:bg-[#fef3e2]"
                      style={{ color: "var(--navy)" }}
                      onClick={(e) => {
                      ...
                      <div>
                        <div
                          className="font-bold text-[0.9rem] uppercase tracking-wide"
                          style={{ fontFamily: "'Rajdhani', sans-serif" }}>
                          {item.en}
                        </div>
                        {item.hi && (
                    ...
                    {/* Mobile children */}
                    {item.children && (
                      <ul className="bg-gray-50/50 pl-6 pb-2">
                        {item.children.map((child, i) => (
                          <li key={i}>
                            <a
                              href={child.url || "#"}
                              className="block px-4 py-2.5 text-sm border-l-2 hover:bg-gray-100/50 transition-colors"
                              style={{
                                color: "var(--navy)",
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, var(--saffron), var(--gold), var(--india-green))",
          }}
        />
      </header>
    </>
